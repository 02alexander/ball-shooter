...
#from calibrater import irl_cord_to_img_point
...
while True:
    ...
    print(rm)
    rm = np.transpose(rm)
    tv = np.dot(-rm, np.transpose(tvecs[0]))
    ...
    #cord = irl_cord_to_img_point(rotated_point)
    #cord = irl_cord_to_img_point(midpoint.reshape(3))
    #cord = irl_cord_to_img_point(np.array([0, 0.089, 0.5]))
    print(midpoint)
